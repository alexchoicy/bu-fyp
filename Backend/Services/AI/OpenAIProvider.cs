...
public class OpenAIProvider : IAIProvider
{
    ...
    public OpenAIProvider(IConfiguration configuration, ILogger<OpenAIProvider> logger, AppDbContext dbContext)
    {
        _logger = logger;
    ...
    public async Task<List<ChatMessage>> GenerateChatResponseAsync(List<Message> chatHistory)
    {
        try
        {
            ...
            {
                Tools = { OpenAIFunctions.ChatTools.GetCourseByCodeAndNumberTool },
            };
                        ...
                        foreach (ChatToolCall toolCall in completion.ToolCalls)
                        {
                            ...
                            switch (toolCall.FunctionName)
                            {
                                case nameof(OpenAIFunctions.DatabaseQueries.GetCourseByCodeAndNumber):
                                {
                                    ...
                                    bool hasCode =
                                        argumentsJson.RootElement.TryGetProperty("code", out JsonElement codeElement);
                                    bool hasCourseNumber = argumentsJson.RootElement.TryGetProperty("courseNumber",
                                    ...
                                    if (!hasCode || !hasCourseNumber)
                                    {
                                        messages.Add(new ToolChatMessage(
                                            toolCall.Id,
                                            ChatMessageContentPart.CreateTextPart("Error: Invalid arguments provided to GetCourseByCodeAndNumber.")));
                                        continue;
                                    }
                                    ...
                                    var toolResult =
                                        await OpenAIFunctions.DatabaseQueries.GetCourseByCodeAndNumber(_dbContext,
                                            codeValue!, courseNumberValue!);
                                    messages.Add(new ToolChatMessage(toolCall.Id,
                                        ChatMessageContentPart.CreateTextPart(JsonSerializer.Serialize(toolResult))));
                                    break;
                                }
                                default:
                                {
    ...
    private sealed class LoggingHandler : DelegatingHandler
    {
        ...
        protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            _logger.LogInformation("OpenAI request {Method} {Uri}\nHeaders: {Headers}\nBody: {Body}", request.Method, request.RequestUri, request.Headers, body);
